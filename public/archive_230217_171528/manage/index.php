...
//Application Title
if(isset($_GET['edit'])){
    $folderName = end(explode('/', trim($_GET['p'])));
    $title_page = str_replace(".php", "",trim($_GET['edit']))."-".$folderName;    
...
$auth_users = array(
    'sajid@phpmaster.com' => '$2y$10$/K.hjNr84lLNDt8fTXjoI.DBp6PpeyoJ.mGwrrLuCZfAwfSAGqhOW' //admin@123
);
    ...
    <div class="path">
        <div class="row">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-end pt-1">
                <a title="<?php echo lng('Back') ?>" class="btn btn-sm btn-outline-primary" onclick="window.history.back()"><i class="fa fa-reply-all"></i> <?php echo lng('Back') ?></a>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($files as $f) {
                ...
                $editme_link = "?p=".urlencode(trim(FM_PATH))."&amp;edit=".urlencode($f)."&env=ace";
                    ...
                    <td data-sort=<?php echo fm_enc($f) ?>>
                        ...
                        <?php
                           if (in_array(strtolower(pathinfo($f, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'))){ ?>
                               ...
                               <?php }else if(in_array(strtolower(pathinfo($f, PATHINFO_EXTENSION)), array('zip', 'rar'))){ ?>
                                    <a href="<?php echo $filelink ?>" title="<?php echo $f ?>">
                               <?php }else{ ?>
                                    <a href="<?php echo $editme_link ?>" target="blank" title="<?php echo $f ?>">
                                <?php } ?>
...
<head>
    ...
    <style>
        ...
        *,*::before,*::after { box-sizing: border-box;}
        ...
        #snackbar { visibility:hidden;min-width:250px;margin-left:-125px;background-color:#00ff008a;color:#fff;font-weight:bolder;text-align:center;border-radius:2px;padding:16px;position:fixed;z-index:1;left:50%;bottom:30px;font-size:17px; }
        #snackbar.show { visibility:visible;-webkit-animation:fadein 0.2s, fadeout 0.8s 2.5s;animation:fadein 0.2s, fadeout 0.8s 2.5s; }
...
<script>
    ...
    // Toast message
    function toast(txt) { var x = document.getElementById("snackbar");x.innerHTML=txt;x.className = "show";setTimeout(function(){ x.className = x.className.replace("show", ""); }, 2000); }
    // Save file
        ...
        if (typeof n !== 'undefined' && n !== null) {
            if (true) {
                ...
                $.ajax({
                    ...
                    beforeSend: function(mes){toast("File Saving..."); window.onbeforeunload = function() {return}},
                    contentType: "application/json; charset=utf-8",
                    success: function(mes){toast("File Saved"); window.onbeforeunload = function() {return}},
