    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div class="card-title">City Table</div>
                <a href="<?= base_url("admin/city/add") ?>" class="btn btn-info">
                    <i class="la la-plus-circle"></i> Add City
                </a>
            </div>
                ...
                <?php if ($data): ?>
                    ...
                    <div class="table-responsive">
                        <table class="table table-head-bg-primary table-striped table-hover text-center">
                            ...
                            <tbody>
                            <?php $i=1; foreach ($data as $value): if(isset($value['name'])):?>
                                <tr>
                                    <td> <?=$i++?> </td>
                                    <td><?=  ucwords($value['name']) ?></td>
                                        
                                    <?php foreach($state1 as $s): ?>
                                        <?php if($s['id']==$value['state_name']): ?>
                                            <td><?= esc(ucwords($s['name'])) ?></td>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                    <td><?=  ucwords($value['page_title']) ?></td>
                                    <td>
                                        <a onclick="return confirm('Are you sure want to delete ? )'" href="<?= base_url() . "/admin/city/delete/" . $value['id'] ?>"
                                           class="btn btn-sm btn btn-rounded btn-danger my-1">
                                            <i class="la la-trash"></i> Delete
                                        </a>
                                        <a href="<?= base_url() . "/admin/city/edit/" . $value['id'] ?>"
